...
def collections_root_kb(
    page: int = 1,
    has_prev: bool = False,     # kept for backward compat, not used
    has_next: bool = False,     # kept for backward compat, not used
    collections: list[tuple[int, str]] | None = None,
) -> InlineKeyboardMarkup:
    """
    Клавиатура корневого меню "Мои коллекции".
    Верхние строки — действия уровня списка:
      - ➕ Новая коллекция
      - 🔑 Добавить по коду
      - 📦 Импорт из CSV/Excel
    Ниже — сами коллекции (с пагинацией) и навигация со счётчиком страниц.
    """
    ...
    nav_row = []
    if page > 1:
        nav_row.append(InlineKeyboardButton(text="⬅️ Назад", callback_data=f"col:list:{page-1}"))
    nav_row.append(InlineKeyboardButton(text=f"Стр. {page}/{total_pages}", callback_data="noop"))
    if page < total_pages:
        nav_row.append(InlineKeyboardButton(text="Вперёд ➡️", callback_data=f"col:list:{page+1}"))
    if nav_row:
        kb.row(*nav_row)
...
def collection_menu_kb(collection_id: int, page: int = 1) -> InlineKeyboardMarkup:
    ...
    if page == 1:
        kb.row(
            InlineKeyboardButton(text="➕ Добавить карточку", callback_data=f"col:add_item:{collection_id}"),
        )
        kb.row(
            InlineKeyboardButton(text="🗂 Посмотреть карточки", callback_data=f"col:list_items:{collection_id}:1"),
            InlineKeyboardButton(text="🎮 Играть", callback_data=f"col:play:{collection_id}"),
        )
        kb.row(
        ...
        kb.row(
            InlineKeyboardButton(text="⬅️ К списку коллекций", callback_data="col:list:1"),
        ...
        kb.row(
            InlineKeyboardButton(text="📥 Импорт карточек (CSV/Excel)", callback_data=f"col:import:items:{collection_id}"),
        )
        kb.row(
...
def item_delete_confirm_kb(item_id: int, collection_id: int) -> InlineKeyboardMarkup:
    b = InlineKeyboardBuilder()
    b.button(text="✅ Да, удалить карточку", callback_data=f"item:delete:confirm:{item_id}")
